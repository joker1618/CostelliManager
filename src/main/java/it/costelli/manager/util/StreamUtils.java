...
/**
 * Created by f.barbano on 17/08/2017.
 */
public class StreamUtils {
	...
	public static <T> String join(T[] source, String separator, Function<T,Object> mapFunc) {
		return join(Arrays.asList(source), separator, mapFunc);
	}
	public static <T> String join(Collection<T> source, String separator, Function<T,Object> mapFunc) {
		return source.stream().map(mapFunc).map(String::valueOf).collect(Collectors.joining(separator));
	}
