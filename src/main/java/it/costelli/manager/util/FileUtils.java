...
public class FileUtils {
	...
	public static Path writeFileSafe(Path outputPath, List<String> lines) throws IOException {
		Path outPath = computeSafelyPath(outputPath);
		writeFile(outPath, lines, null, false);
		return outPath;
	}
	...
	public static Path versionize(Path targetPath) throws IOException {
		...
		while(newPath == null || Files.exists(newPath)) {
			v++;
			newPath = folder.resolve(String.format("%s.v%02d", fileName, v));
		}

		moveFile(targetPath, newPath, false);
		return newPath;
	}
	...
	public static List<Path> readPathsFromFile(Path sourceFile) throws IOException {
